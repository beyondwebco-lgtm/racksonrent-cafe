...
export default function Footer() {
  return (
    <footer className="border-t border-[#650000] bg-[#4A0000] text-white py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Main Footer Row */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 pb-8 border-b border-[#650000]/60">
          ...
          <nav className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 text-xs sm:text-sm font-bold">
            {NAV_LINKS.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-[#FFFBCC] hover:text-[#FAFA33] transition-colors py-1 px-1.5 rounded active:scale-95"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 text-xs sm:text-sm font-bold">
            <a
              ...
              className="inline-flex items-center gap-1.5 text-[#FFFDF5] hover:text-[#FAFA33] transition-colors py-1 px-1.5 rounded active:scale-95"
            >
              <Phone className="w-4 h-4 text-[#FAFA33] flex-shrink-0" />
              <span>{SITE_CONFIG.phoneDisplay}</span>
            </a>

            <span className="text-[#650000] hidden xs:inline">•</span>
